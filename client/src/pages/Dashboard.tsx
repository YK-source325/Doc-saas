  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-20">
      ...
      <div className="mt-12">
        ...
        {stats.data && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "SCORE MEDIO", value: stats.data.averageFinalScore.toFixed(2), color: "#8F6F25" },
      ...
      <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* ALLARMI REVOCA */}
        <FadeInSection>
          <div className="border border-[#f59e0b]/40 bg-white p-6 h-full">
                  ...
                  <tbody className="divide-y divide-black/10">
                    {alerts.data.map((a) => (
                      <tr
                        ...
                        className="hover:bg-[#A8842C]/5 cursor-pointer"
                      >
                        <td className="py-3 pr-2 font-serif">{a.name}</td>
                        <td className="py-3 pr-2 font-brand text-xl text-[#8F6F25]">
                          {a.finalScore.toFixed(2)}
                        </td>
                        <td className="py-3 pr-2"><StatusBadge status={a.plaqueStatus} /></td>
                        <td className="py-3 text-[#141414]/60">{a.renewalDueAt ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <FadeInSection delay={150}>
          <div className="border border-black/10 bg-white p-6 h-full">
            <div className="flex items-center gap-3">
              <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none" stroke="#A8842C" strokeWidth="2">
                <line x1="5" y1="20" x2="5" y2="12" />
                <line x1="12" y1="20" x2="12" y2="6" />
                <line x1="19" y1="20" x2="19" y2="14" />
              </svg>
              <h2 className="font-serif font-semibold text-2xl text-[#141414]">LEADERBOARD LIVE</h2>
            </div>
            <div className="mt-6">
                  ...
                  {leaderboard.data.map((entry) => {
                    ...
                    return (
                      <li
                        ...
                        className="flex items-center gap-4 py-3 px-2 hover:bg-[#A8842C]/5 cursor-pointer"
                      >
                        <span className="font-brand text-xl text-[#A8842C] w-8">#{entry.rank}</span>
                        <div className="flex-1 min-w-0">
                          <p className="font-serif truncate">{entry.name}</p>
                          <p className="text-xs text-[#141414]/40">{entry.city}</p>
                        </div>
                        <span className="font-brand text-2xl text-[#8F6F25]">
