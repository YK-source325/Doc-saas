...
const inputClass =
  "w-full bg-[#FAF8F4] border border-black/15 px-3 py-2 text-sm text-[#141414] focus:border-[#A8842C] focus:outline-none";
...
function SubscriptionRequests() {
  ...
  return (
    <FadeInSection className="mt-16">
      <h2 className="font-serif font-semibold text-3xl text-[#141414]">RICHIESTE DI ABBONAMENTO</h2>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-black/10">
              {requests.data.map((r) => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-[#A8842C]/5 align-top">
                    <td className="px-4 py-3">
                      <p className="font-serif text-base text-[#141414]">{r.structureName}</p>
                      {r.message && (
                        <p className="mt-1 font-serif italic text-xs text-[#141414]/50 max-w-xs">
                          {r.message}
                        </p>
                      )}
                    </td>
                    <td className="px-4 py-3 text-[#141414]/70">
                      {r.contactName}
                      <span className="block text-xs text-[#141414]/40">{r.email}</span>
                      {r.phone && <span className="block text-xs text-[#141414]/40">{r.phone}</span>}
                    </td>
                    <td className="px-4 py-3 text-[#141414]/60">{r.city}</td>
                    <td className="px-4 py-3 font-brand text-lg text-[#A8842C]">{r.tier}</td>
                    <td className="px-4 py-3">
                      <span
                    ...
                    <td className="px-4 py-3">
                      <select
                        ...
                        className="bg-[#FAF8F4] border border-black/15 px-2 py-1 text-xs text-[#141414] focus:border-[#A8842C] focus:outline-none"
                      >
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-20">
      <FadeInSection>
        ...
        <div className="mt-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="font-serif font-semibold text-5xl sm:text-6xl text-[#141414]">
              PANNELLO SVILUPPATORE
            </h1>
            <p className="mt-2 font-serif italic text-xl text-[#141414]/60">
              Gestione completa delle strutture e dello stato delle targhe.
            </p>
          </div>
          <button
            onClick={openNew}
            className="px-6 py-3 bg-[#A8842C] text-white font-bold uppercase tracking-widest text-xs hover:bg-[#8F6F25] transition-colors"
          >
            + Nuova Struttura
          </button>
        </div>
        ...
        {places.data && (
          <div className="overflow-x-auto border border-black/10 bg-white">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-black/10">
                {places.data.map((p) => (
                  <tr key={p.id} className="hover:bg-[#A8842C]/5">
                    <td className="px-4 py-3 font-serif text-base">{p.name}</td>
                    <td className="px-4 py-3 text-[#141414]/60">{p.city}</td>
                    <td className="px-4 py-3 font-brand text-xl text-[#8F6F25]">
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      <button
                        onClick={() => openEdit(p)}
                        className="px-3 py-1 border border-[#A8842C]/40 text-[#A8842C] text-[10px] uppercase tracking-widest hover:bg-[#A8842C]/10"
                      >
                        Modifica
                      </button>
                      <button
      ...
      {editing && (
        <div className="fixed inset-0 z-[60] bg-[#FAF8F4]/90 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="w-full max-w-lg bg-white border border-black/15 p-8">
            <h2 className="font-serif font-semibold text-2xl text-[#141414]">
              {editing === "new" ? "NUOVA STRUTTURA" : `MODIFICA — ${editing.name}`}
            </h2>
            <form onSubmit={submit} className="mt-6 space-y-4">
              <div>
                <label className="text-[10px] uppercase tracking-widest text-[#141414]/50">Nome</label>
                <input className={`mt-1 ${inputClass}`} value={form.name} onChange={set("name")} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-[#141414]/50">Categoria</label>
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-[#141414]/50">Città</label>
                  <input className={`mt-1 ${inputClass}`} value={form.city} onChange={set("city")} />
                </div>
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-widest text-[#141414]/50">Indirizzo</label>
                <input className={`mt-1 ${inputClass}`} value={form.address} onChange={set("address")} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-[#141414]/50">Data ispezione</label>
                  <input type="date" className={`mt-1 ${inputClass}`} value={form.inspectedAt} onChange={set("inspectedAt")} />
                </div>
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-[#141414]/50">Score Revisore (0-5)</label>
                  <input type="number" step="0.1" min="0" max="5" className={`mt-1 ${inputClass}`} value={form.revisoreScore} onChange={set("revisoreScore")} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-[#141414]/50">Score Google (opz.)</label>
                  <input type="number" step="0.1" min="0" max="5" className={`mt-1 ${inputClass}`} value={form.googleScore} onChange={set("googleScore")} />
                </div>
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-[#141414]/50">Score TripAdvisor (opz.)</label>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="px-5 py-2 border border-[#F0EADB]/30 text-[#141414]/60 text-xs uppercase tracking-widest hover:border-[#F0EADB]/60"
                >
                  Annulla
                </button>
                <button
                  type="submit"
                  disabled={pending}
                  className="px-5 py-2 bg-[#A8842C] text-white font-bold text-xs uppercase tracking-widest hover:bg-[#8F6F25] disabled:opacity-50"
                >
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-[60] bg-[#FAF8F4]/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white border border-[#ef4444]/40 p-8 text-center">
