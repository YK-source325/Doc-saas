    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-20">
      <FadeInSection>
        ...
        <div className="mt-6 flex items-center gap-6">
          ...
          <div>
            <h1 className="font-serif font-semibold text-4xl sm:text-5xl text-[#141414]">{user?.name}</h1>
      ...
      <FadeInSection className="mt-12">
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white border border-black/10 p-6">
            <p className="font-brand text-5xl text-[#141414]">{ratings.data?.length ?? "—"}</p>
            <p className="mt-2 text-[10px] tracking-widest text-[#141414]/40">VALUTAZIONI LASCIATE</p>
          </div>
          <div className="bg-white border border-black/10 p-6">
            <p className="font-brand text-5xl text-[#A8842C]">
              {average !== null ? average.toFixed(2) : "—"}
            </p>
            <p className="mt-2 text-[10px] tracking-widest text-[#141414]/40">IL TUO VOTO MEDIO</p>
          </div>
        </div>
      </FadeInSection>

      <FadeInSection className="mt-12">
        <h2 className="font-serif font-semibold text-2xl text-[#141414]">LE MIE VALUTAZIONI</h2>
            ...
            {ratings.data.map((r) => (
              <li key={r.id} className="p-5 hover:bg-[#A8842C]/5 transition-colors">
                <Link to={`/places/${r.placeId}`} className="flex items-center gap-5">
                  <span className="font-brand text-3xl text-[#A8842C] w-16 shrink-0">
                    {r.score.toFixed(1)}
                  </span>
                  <div className="min-w-0">
                    <p className="font-serif text-lg text-[#141414] truncate">
                      {r.placeName} <span className="text-[#141414]/40 text-sm">· {r.placeCity}</span>
                    </p>
                    {r.comment && (
                      <p className="mt-0.5 font-serif italic text-sm text-[#141414]/60 truncate">
                        {r.comment}
                      </p>
                    )}
                  </div>
