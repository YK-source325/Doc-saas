...
export default function LiveTicker() {
  ...
  return (
    <div className="border-y border-black/10 bg-white/60 overflow-hidden">
      <div className="marquee-track py-3">
        {strip.map((entry, i) => (
          ...
          >
            <span className="text-[10px] tracking-[2px] text-[#A8842C]">LIVE</span>
            <span className="font-serif text-base text-[#141414]/80 group-hover:text-[#141414] transition-colors">
              {entry.name}
            </span>
            <span className="font-brand text-lg text-[#141414]">{entry.finalScore.toFixed(2)}</span>
            <span
              className="text-sm"
            ...
            <span className="text-[#141414]/15 pl-8">·</span>
          </button>
        ))}
      </div>
    </div>
