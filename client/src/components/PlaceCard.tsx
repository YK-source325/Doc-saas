...
export default function PlaceCard({ place }: { place: EnrichedPlace }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs text-[#F0EADB]/50">
        {TYPE_LABELS[place.type] ?? place.type} · {place.city}
      </p>
      <div className="my-6 flex flex-col items-center gap-2">
        <ScoreGauge score={place.finalScore} size={80} />
        <span className="text-[10px] tracking-widest text-[#F0EADB]/40">LIVE SCORE</span>
      </div>
      <span className="mt-auto text-xs text-[#C9A84C] font-bold tracking-widest">
        DETTAGLI ANALISI →
      </span>
    </div>
  );
