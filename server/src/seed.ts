...
async function seed() {
  ...
  await db.insert(places).values(PLACES);
  await db.insert(communityRatings).values(RATINGS);
  ...
  console.log(`Seed completato: ${PLACES.length} strutture, ${RATINGS.length} valutazioni, ${USERS.length} utenti.`);
  process.exit(0);
}
